...
class NeuralNetwork():
    ...
    def train_graph(self, sess, x_train, y_train, x_valid, y_valid, n_epoch=1, 
        ...
        print('\tnn_name = {}'.format(self.nn_name))
        print('\tn_epoch = {}'.format(n_epoch))
        print('\tmb_size = {}'.format(self.mb_size))
        print('\tlearn_rate = {:.10f}'.format(self.learn_rate))
        print('\tlearn_rate_0 = {:.10f}'.format(self.learn_rate_0))
        print('\tlearn_rate_alpha = {:.10f}'.format(self.learn_rate_alpha))
        print('\tlearn_rate_step = {}'.format(self.learn_rate_step))
        print('\tlearn_rate_pos = {}'.format(self.learn_rate_pos))
        print('\tdropout_proba = {}'.format(self.dropout_proba))
    ...
    def save_model(self, sess):
        ...
        filepath = os.path.join(os.getcwd(), SAVES_DIR_NAME, 
                                self.nn_name+'_params.npy')
    ...
    def load_session_from_file(self, filename):
        ...
        tf.reset_default_graph()
        filepath = os.path.join('.', SAVES_DIR_NAME, filename)
        #filepath = './'+SAVES_DIR_NAME+'/'+filename + '.meta'
        saver = tf.train.import_meta_graph(filepath+'.meta')
        sess = tf.Session() # default session
        #saver.restore(sess, filename) # restore session
        saver.restore(sess, filepath) # restore session
